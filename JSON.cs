...
{
	/// <summary>
	/// This class encodes and decodes JSON strings.++++caution only basic type is supported
	/// Spec. details, see http://www.json.org/
	///
	/// JSON uses Arrays and Objects. These correspond here to the datatypes ArrayList and Dictionary<string,object>.
	/// All numbers are parsed to long and double.
	/// </summary>
	class JSON
	{
		...
		/// <returns>An ArrayList, a Dictionary<string,object>, a int, a string, null, true, or false</returns>
		...
		/// <summary>
		/// Converts a Dictionary<string,object> / ArrayList object into a JSON string
		/// </summary>
				...
				if (c == '"') {
					...
					if (c == '"') {
						...
						if (remainingLength >= 4) {
							...
							s.Append(Convert.ToChar((int)codePoint));
							// skip 4 chars
							index += 4;
						} else {
							break;
        ...
        protected static ValueType ParseNumber(char[] json, ref int index, ref bool success)
        {
            ...
            int i = 0;
            string temp = new string(json, index, charLength);
            
            success = long.TryParse(new string(json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture, out long_number);

            if(success == false)
            {
                i++;
                success = double.TryParse(new string(json, index, charLength), NumberStyles.Any, CultureInfo.InvariantCulture, out double_number);
            }
            index = lastIndex + 1;
		...
		protected static bool SerializeValue(object value, StringBuilder builder)
		{
			...
			if (value is string) {
                ...
                //DebugUtil.Log("unsppoerted value type " + value.GetType().ToString());
			} else if (value == null) {
				builder.Append("null");
			} else {
				success = false;
		...
		protected static bool SerializeObject(Dictionary<string,object> anObject, StringBuilder builder)
		{
            ...
            foreach (KeyValuePair<string, object> pair in anObject)
            {
                string key = pair.Key;
